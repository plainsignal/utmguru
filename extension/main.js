...
async function getItems() {
    return chrome.storage.local.get(['utm_urls']).then(result => {
        return result.utm_urls ? JSON.parse(result.utm_urls) : [];
    });
}

async function setItems(items) {
    chrome.storage.local.set({'utm_urls': JSON.stringify(items)});
}
...
async function renderSavedList() {
    ...
    if (items.length === 0) {
        ...
        items.forEach(({ desc, url }, index) => {
            ...
            li.classList.add('border', 'border-gray-200', 'rounded', 'p-4', 'bg-gray-50');
            li.innerHTML = `
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 divide-gray-200 gap-x-3">
          <div class="flex-1">
            ...
            <div class="flex items-center gap-2">
              ...
              <button class="copy-link text-xs bg-black text-white px-2 py-1 rounded hover:bg-gray-800 cursor-pointer" data-url="${url}">Copy</button>
            </div>
          </div>
          <button data-index="${index}" class="delete-btn text-red-600 hover:text-red-800 text-xs cursor-pointer">DELETE</button>
        </div>`;
...
const explainers = {
    source: "The <b>Source</b> (utm_source) identifies where your traffic comes from. ",
    medium: "The <b>Medium</b> (utm_medium) describes the marketing channel.",
    campaign: "The <b>Campaign</b> (utm_campaign) is the name of your promotion or campaign.",
    term: "The <b>Term</b> (utm_term) is used for paid search keywords.",
    content: "The <b>Content</b> (utm_content) helps differentiate ads or links pointing to the same URL."
};
...
function generateUTMUrl() {
    let base = document.getElementById('base-url').value.trim();
    const params = [];
...
document.querySelectorAll('.utm-shortcut').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        document.querySelectorAll('.utm-shortcut').forEach(b => {
